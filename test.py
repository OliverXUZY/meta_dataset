...
def main(config):
  ...
  ##### Model #####
  utils.log("config path: {}".format(config['path']))
  if config.get('ckpt'):
  ...
  utils.log("test on domains: {}".format(config['test_set_args']['domains']), filename='test.txt')
...
if __name__ == '__main__':
  ...
  if args.log_path:
    config['path'] = args.log_path
    utils.log("set log path: {}".format(config['path']))
  ...
  if args.domains:
    config['test_set_args']['domains'] = [args.domains]
    print("test on domains {}".format(args.domains))
